...
// Full 12-swara chromatic scale with ratios (Madhya octave)
...
// Build three-octave pool: mandra (ratio/2), madhya (ratio), taar (ratio*2)
// octave: 0=mandra, 1=madhya, 2=taar
// absoluteSemitone used for jump calculation: mandra=semitone-13, madhya=semitone, taar=semitone+13
  ...
  [0, 1, 2].flatMap(oct =>
    baseIdxArr.map(i => {
      ...
      return {
        ...b,
        octave: oct,
        ratio: b.ratio * ratioMult,
        // absolute semitone across octaves for jump logic
        absSemitone: b.semitone + (oct - 1) * 13,
      };
    })
  );

// Shuddha pool indices
...
function generateCards(levelIdx) {
  ...
  // Key for dedup: semitone + octave combined
  ...
  for (let i = 0; i < ACTIVE_BEATS; i++) {
    ...
    let candidates = pool.filter(s => {
      ...
      if (prev !== null) {
        // Use absSemitone for cross-octave jump measurement
...
function useAudioEngine() {
  ...
  const scheduleBeats = useCallback((bpm, totalBeats, onBeat, onDone) => {
    ...
    const tick = () => {
      while (nextBeatRef.current < ctx.currentTime + schedAhead && scheduled < totalBeats) {
        ...
        const capturedBeat = beat, capturedT = t, capturedSched = scheduled;
        setTimeout(() => onBeat(capturedBeat % 4, isDown, capturedSched, capturedT), delay);
      ...
      if (scheduled < totalBeats) {
        ...
        // Fire onDone after last beat completes
        const lastBeatTime = nextBeatRef.current - (60 / bpm);
        const doneDelay = Math.max(0, (lastBeatTime - (ctxRef.current?.currentTime || 0)) * 1000) + 300;
  ...
  const updateDroneFreq = useCallback((freq) => {
    ...
    [freq, freq*2, freq*3, freq*5, freq*1.5, freq*3].forEach((f, i) => {
      try { droneNodesRef.current[i]?.frequency.setTargetAtTime(f, t, 0.1); } catch(e){}
    });
  }, []);

  return { startDrone, stopDrone, scheduleBeats, stopScheduler, resumeCtx, updateDroneFreq, playGuruNote, getCtx };
}

// ─── Sub-components ───────────────────────────────────────────────────────────
function SwaraCard({ swara, state }) {
  // state: "dim" | "idle" | "active"
  // octave: 0=mandra (underline), 1=madhya (plain), 2=taar (dot above)
  const oct = swara.octave ?? 1;
  return (
    <div className={"swara-card card-" + state}>
      <span className="card-dv">{swara.dv}</span>
      <span className={"card-name" + (oct === 0 ? " oct-mandra" : oct === 2 ? " oct-taar" : "")}>
        {swara.short}
    ...
    <div className="beat-dots">
      {[0,1,2,3].map(i => (
        <div key={i} className={"beat-dot" +
          (active && beat === i ? (i===0 ? " dot-dn" : " dot-up") : "")} />
      ))}
    </div>
  );
}
...
export default function SwaraSlamApp() {
  ...
  // ── Progression
  const [level,        setLevel]        = useState(0);   // 0-2
  const [setNum,       setSetNum]       = useState(0);   // 0-4
  const [cards,        setCards]        = useState(() => generateCards(0));
  const [currentCards, setCurrentCards] = useState(null); // frozen for retry

  // ── Playback state
  const [phase,        setPhase]        = useState("idle");    // idle | leadin | active | done
  const [activeCard,   setActiveCard]   = useState(-1);
  const [dotBeat,      setDotBeat]      = useState(-1);

  // ── BPM
  const [bpm,          setBpm]          = useState(BASE_BPM);
  const [manualBpm,    setManualBpm]    = useState(false);     // user overrode slider
  ...
  // Compute auto-BPM for current set
  ...
  const advanceSet = useCallback((lvl, sn) => {
    ...
    if (nextSet >= SETS_PER_LEVEL) {
      ...
      if (nextLevel >= LEVEL_CONFIG.length) {
        // Game complete — show All Levels Up screen
        setAllLevelsUp(true);
        setConfetti(true);
        ...
        setTimeout(() => {
          ...
          const newCards = generateCards(nextLevel);
          setCards(newCards); setCurrentCards(null);
  ...
  const handleRestart = useCallback(() => {
    setAllLevelsUp(false);
    setConfetti(false);
    setLevel(0); setSetNum(0);
    const newCards = generateCards(0);
    setCards(newCards); setCurrentCards(null);
  ...
  // Phase-derived card state
  const getCardState = (i) => {
    if (phase === "idle" || phase === "done") return "dim";
    if (phase === "leadin") return "idle";
    if (phase === "active") return i === activeCard ? "active" : "idle";
    return "idle";
  };

  const phaseLabel = phase === "leadin"
    ? "Get Ready…"
    : phase === "active"
    ? "Sing Along 🎵"
    : phase === "done"
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .ss-app {
          min-height:100vh; background:#F9F7F2;
          font-family:'DM Sans',sans-serif; color:#1C1A17;
          display:flex; flex-direction:column; align-items:center;
          padding:0 1.25rem 2.5rem; overflow-x:hidden;
        }
        ...
        .prog-badge{
          ...
          color:#9A7B50;font-weight:500;white-space:nowrap;
        }
        .prog-badge strong{color:#1C1A17;font-weight:600}
        .prog-dots{display:flex;gap:5px;align-items:center}
        .prog-dot{
          width:7px;height:7px;border-radius:50%;
          background:rgba(0,0,0,.1);transition:background .25s,transform .25s;
        }
        ...
        .phase-label{
          ...
          color:rgba(0,0,0,.3);white-space:nowrap;
        }
        ...
        .bpm-flash{
          ...
          font-family:'Cormorant Garamond',serif;font-size:clamp(32px,7vw,52px);font-weight:600;
          padding:.4em .9em;border-radius:12px;
          pointer-events:none;z-index:150;
          opacity:0;transition:none;
        }
        .bpm-flash-in{
          animation:bpmPop 1.5s cubic-bezier(.34,1.56,.64,1) forwards;
        }
        @keyframes bpmPop{
        ...
        .arena-field{
          width:100%;max-width:480px;
          border:.5px solid rgba(0,0,0,.07);border-radius:16px;
          display:flex;flex-direction:column;align-items:center;justify-content:center;
          gap:10px;padding:16px 14px;position:relative;
          background:rgba(255,255,255,.4);transition:border-color .12s,background .12s;
        }
        .arena-field::before,.arena-field::after{
          content:'';position:absolute;width:14px;height:14px;
          border-color:rgba(192,95,47,.18);border-style:solid;
        }
        ...
        .swara-card{
          ...
          transition:transform .09s cubic-bezier(.34,1.56,.64,1),box-shadow .09s,border-color .09s,background .09s,opacity .2s;
        }
        ...
        /* Taar: small dot above via superscript span */
        ...
        .card-active{
          transform:scale(1.1);
          background:#FFF3EE;border-color:#C05F2F;
          box-shadow:0 0 0 2.5px rgba(192,95,47,.4),0 6px 22px rgba(192,95,47,.25),0 2px 6px rgba(0,0,0,.07);
        }
        ...
        .levelup-overlay{
          ...
          background:rgba(249,247,242,.92);backdrop-filter:blur(6px);
          animation:fadeIn .3s ease both;
        }
        .levelup-ornament{font-family:'Cormorant Garamond',serif;font-size:14px;color:rgba(0,0,0,.18);letter-spacing:.35em}
        .levelup-title{font-family:'Cormorant Garamond',serif;font-size:clamp(52px,12vw,88px);font-weight:600;color:#C05F2F;font-style:italic;animation:titlePop .5s .1s cubic-bezier(.34,1.56,.64,1) both}
      ...
      {levelUpVisible && (
        <div className="levelup-overlay">
          <p className="levelup-ornament">ॐ &nbsp; स &nbsp; र &nbsp; ग &nbsp; म</p>
          <div className="levelup-title">Level Up!</div>
          <p className="levelup-sub">
            Entering Level {level + 2} — {LEVEL_CONFIG[Math.min(level + 1, 3)].label}
          </p>
        </div>
      )}
      ...
      {allLevelsUp && (
        <div className="levelup-overlay">
          <div className="levelup-title" style={{fontSize:"clamp(42px,10vw,72px)"}}>All Levels Up!</div>
          <p className="levelup-sub" style={{marginBottom:"0.5rem"}}>You have mastered all four levels</p>
      ...
      <div className="ss-app" ref={appRef}>
        ...
        <div className="progress-bar">
          <span className="prog-badge">
            <strong>Level {level + 1}</strong> · {LEVEL_CONFIG[level].label}
          </span>
        ...
        <section className="ss-controls" aria-label="Practice controls">
          <div className="ctrl-row">
            ...
            <input type="range" className="ss-slider"
          ...
          {/* Play row: Back | Play | Next */}
          <div className="play-row">
            <button className="nav-btn" onClick={handleRetry} disabled={isPlaying} aria-label="Try again" title="Try Again">
              <SkipBack />
            </button>
            <button className={"play-btn"+(isPlaying?" playing":"")} onClick={togglePlay} aria-label={isPlaying?"Stop":"Play"}>
              {isPlaying ? <Pause /> : <Play />}
            </button>
            <button className="nav-btn" onClick={handleNextSet} disabled={isPlaying} aria-label="Next set" title="Next Set">
              <SkipFwd />
            </button>
          </div>
        </section>

        <p className="module-tag">Module 03 — Progression System</p>
      </div>
    </>
  );
}
